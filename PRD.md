...
| **N1 · Ingesta** | Clasificar URL (regex) → fast path (Shopify `.json`, JSON-LD, OG) → Firecrawl `/scrape` (markdown+images+branding+product+screenshot, `onlyMainContent`) → merge. Mini-crawl opcional de 2–3 páginas internas (`/reviews`, `/faq` — mejor fuente de objeciones). **Se captura la URL FINAL servida** (tras redirecciones) junto a la pedida: si difieren de forma significativa (cambio de host, o ruta profunda → raíz), CP1 lo AVISA *(2026-07-14, T2.7)*. Con texto libre: se salta el scraping y el texto es el contenido base | HTTP propio + Firecrawl (fallback Jina) | — | $0,002–0,02 |
...
| **N3 · ProductBrief** | Síntesis multifaceta en UNA llamada con structured output conforme al schema del Apéndice A: producto, beneficios, audiencia (segmentos + awareness), pains, **objeciones con contraargumento**, social proof, marca (+`banned_or_risky_claims`), pricing, assets clasificados, 5–10 ángulos. Validación determinista post-parse: precio N1==N3 (solo perfil `url`), imagen hero, hooks ≤12 palabras, toda `suggested_assets[]` ∈ `assets.images[]` (los inválidos se eliminan con warning). La falta de imagen hero es en AMBOS perfiles una **decisión de CP1**, no un fallo del run: CP1 pide imágenes de forma bloqueante O el usuario elige derivar N7a a packshot IA; en perfil `url`, si el scraping trajo imágenes clasificadas `broll`, puede además promover una a hero *(cambio 2026-07-13, T1.15: antes el perfil `url` fallaba el run en N3 — pero una web de servicio/SaaS sin packshot es un caso legítimo, no un scraping roto, y el fallo duro tiraba la síntesis ya pagada)* | Claude Sonnet 5 + prompt caching + anti-injection | ✅ **CP1: brief editable campo a campo**, con marcas extraído-vs-inferido (`evidence`/`confidence`) | $0,07–0,10 |
...
- **URL pedida vs URL servida** (T2.7, 2026-07-14): `RawContent` guarda LAS DOS (`url` + `urlFinal`). Qué expone cada camino, **verificado contra los servicios reales** (no contra las docs): el **fast path** la trae en `response.url`; **Firecrawl** en **`metadata.url`** — **NO** en `metadata.sourceURL`, que ECHOEA la pedida pese al nombre; **Jina** (fallback) **NO PUEDE detectarla**: su preámbulo `URL Source:` también ECHOEA la pedida (sirviendo la home, sigue diciendo la URL del producto), así que ese camino registra `urlFinal = null` y no avisa — un dato no observado se declara, no se inventa. Un comparador de criterio ESTRECHO marca solo lo que cambia qué se analizó: **cambio de host**, **destino = home desnuda** y **la rama del path diverge** (el directorio PADRE de lo pedido no sobrevive: `/products/x` → `/collections/y`, la categoría a la que una tienda manda un producto descatalogado). NO se marcan las benignas (`http→https`, `www.`, barra final, query/`utm_*`, locale, subdominio y el **rename del slug dentro del mismo padre** — la redirección legítima más frecuente): un aviso que sale siempre no lo lee nadie. **Decisión de producto: AVISA, NO BLOQUEA** — el run sigue, CP1 muestra «se analizó otra página» con las dos URLs y el humano decide (mismo precedente que T1.15: matar el run le quita al humano la decisión que el checkpoint existe para darle; hay redirecciones legítimas —un producto renombrado, una web reestructurada— que solo él puede juzgar). Origen: el análisis de `dr-squatch.com/products/pine-tar-bar-soap`, que hace `301` a la home, producía un brief de la home sin que nadie se enterase; el caso NORMAL es un producto descatalogado.
...
- `BriefValidator`: checks deterministas (precio N1==N3, hero image, longitud de hooks, enums, `suggested_assets[] ∈ assets.images[]`, **redirección significativa de la URL analizada** —warning `url_redirected`, T2.7, §9.1: avisa, no bloquea— ) con **perfil por origen** (`url` incluye el cross-check de precio; `manual` lo omite; la falta de hero image es **decisión de CP1 en ambos perfiles**, no un error — *cambio 2026-07-13, T1.15; antes `url` fallaba el run*) + warnings accionables ("no pudimos leer la página; sube 3 imágenes y una descripción" — patrón de doble entrada de Prizmad). Las **cardinalidades del schema** (5–10 ángulos, 2–3 hooks…) se garantizan aquí en la capa Zod: los structured outputs de Anthropic no aplican constraints de array (§13.2).
...
| Word timestamps (para captions karaoke) | ASR `fal-ai/elevenlabs/speech-to-text` ($0,03/min) — **ruta por defecto ✓T4.5** (Kokoro CONFIRMADO sin timestamps nativos → ASR obligatorio). Para ElevenLabs, la capacidad nativa queda `[verificar]` (Turbo expone un param `timestamps` NO ejercitado; si emite nativos, ahorraría el ASR en Standard/Premium) | ídem | ídem |
...
- **Deuda de verificación** (heredada de `research/01 §8.11`): enums exactos de `aspect_ratio` en Kling v3/Wan 2.6/HappyHorse, y si los endpoints TTS de fal (ElevenLabs/Kokoro) devuelven **word timestamps nativos** (hasta confirmarlo, la ruta de timestamps es el ASR). Se cierra en la primera tarea de integración de cada modelo (el planning lo recogerá).
  - **Enums de b-roll — PARCIALMENTE cerrado en T4.8 (2026-07-17, integración N7d contra fal real)**: el b-roll v1 se integró con **Veo 3.1** (no Kling v3/Wan 2.6, que NO están sembrados — 404 en fal). Enums EXACTOS verificados vs el openapi de fal y sembrados en `model_profile` (`capabilities.aspects/durations/resolutions`): `fal-ai/veo3.1/image-to-video` (i2v) → aspects `[auto,9:16,16:9]`, duration `[4,6,8]s`, resolution `[720p,1080p,4k]`; `fal-ai/veo3.1/reference-to-video` (r2v) → aspects `[9:16,16:9]`, duration fijo `[8]s`, resolution `[720p,1080p,4k]`. La deuda de enums de **Kling v3/Wan 2.6/HappyHorse sigue viva** (se cierra cuando/si se integren esos modelos). Evidencia en vivo: `docs/verifications/T4.8/`.
  - **`[verificar]` word timestamps nativos — PARCIALMENTE cerrado en T4.5 (2026-07-17, cadena TTS→ASR real contra fal)**:
    - **Kokoro (Test): CONFIRMADO sin timestamps nativos** — su output es solo `{audio:{url}}` (no existe campo de timestamps en su contrato). → el ASR es OBLIGATORIO para el tier Test.
    - **ElevenLabs Turbo v2.5 (Standard): capacidad nativa NO verificada** — Turbo EXPONE un parámetro `timestamps` que NO se ejercitó en T4.5 (se observó `null` bajo los params por defecto, que NO prueba que el modelo no pueda emitirlos). Si a petición emite alineación nativa, se AHORRARÍA el ASR $0,03/min en Standard/Premium (el ahorro COGS que este `[verificar]` persigue). Pedir-y-comprobar el param queda como follow-up (candidato: T4.6 preview de voz, o T4.11). El ASR se mantiene como ruta por defecto mientras tanto.
    - **Eleven v3 (Premium): deferred** (no se corrió en vivo).
    - Evidencia en vivo: `docs/verifications/T4.5/` (el jsonb sellado trae el shape del ASR `speech-to-text`, no del TTS). `model_profile` no tiene columna de nota (solo `verified_at`).
  - **Deuda de seed detectada (T4.5)**: §13.1 escribe el TTS Test como `fal-ai/kokoro/{spanish,…}`, pero lo sembrado en T3.4 es el endpoint BASE `fal-ai/kokoro`, cuyo `VoiceEnum` es solo `af_/am_` (inglés). El español NO puede servirse por el tier Test tal como está sembrado (va por ElevenLabs Turbo, tier Standard, 2,5× el coste). Sembrar el/los endpoint(s) de idioma de la familia Kokoro —si existen— es follow-up de T4.11/T3.4.
  ...
  - **`[verificar]` de `fal-ai/veo3.1/image-to-video` CERRADO EN VIVO (T5c.7, 2026-08-14, con dinero real)**: el perfil estaba sembrado `"unverified": true` (`model-profiles.json:164`) — precio 20¢/s y payload NO contrastados contra la API real. La verificación de T5c.7 (primer vídeo end-to-end real) lo ejerció con fal REAL en DOS nodos (N7d body i2v 6s = 120c, N7f CTA i2v 4s = 80c) → **precio $0,20/s CONFIRMADO** y payload `{prompt, duration:"Ns", image_url, resolution:"720p", aspect_ratio:"9:16", generate_audio:false}` → `{video:{url,…}}` confirmado. El seed pasa a `"unverified": false`. `unverified` es metadato informativo (no gatea gasto en runtime).
  - **DECISIÓN DE PRODUCTO — tier por defecto (T5c.4, 2026-08-13, regla de trabajo 3)**: el tier por defecto del lote **sigue siendo `test`** (`plan-batch.ts:44` `defaultBatchConfig`), NO se cambia a `premium`. Contexto: solo `premium` tiene hoy endpoints i2v vivos (`isTierGenerationReady`→true); `test`/`standard` dejan el b-roll como etiqueta pendiente-F4 y no generan vídeo. La tentación (tras el primer uso real en prod, donde el usuario corrió 7 lotes `test` y obtuvo 0 vídeos) era hacer que `test` generase o que el default fuese `premium`. **Ambas rechazadas**: (a) cablear i2v a `test` convierte el tier de exploración barato en uno que gasta ~$3-7 en CADA aprobación y rompe la premisa del money-guard/CP2 (`test`=barato); (b) default `premium` hace que el flujo por-defecto gaste sin intención. La solución elegida es **default `test` + hacer la elección de `premium` consciente**: el selector de tier de CP2 ya muestra el coste al vuelo, y T5c.2 avisa (post-aprobación, en `RunHeader`) cuando un tier no genera vídeo. Si en el futuro se quiere que `test`/`standard` generen, es cambio de alcance MAYOR (el default pasa a gastar) → volver a preguntar con la consecuencia explícita.
...
1. **Orgánico puro con sonido trending nativo (la que captura el deseo — con condiciones)**: el master se exporta **sin música quemada** pero con "music headroom" (voz normalizada a −14 LUFS con espacio dinámico). El **Trending Sound Advisor** (N10) consulta TikTok Creative Center (Popular Music) y sugiere sonidos trending compatibles con el mood; el sonido se añade **de forma nativa en la app al publicar**. Condiciones que la herramienta muestra y valida: (a) los trending sounds del catálogo general solo están disponibles en **cuentas personales/creator** — en cuentas Business de TikTok el picker se limita a la Commercial Music Library, y el path de publicación por API (D4) usa precisamente cuentas Business, así que el deseo D11 completo solo se cumple publicando manualmente desde una cuenta personal/creator; (b) un post con sonido trending no-CML **no puede promocionarse después** (ver punto 2). El equivalente de Instagram (acceso a música limitado en cuentas Business según región) — deuda `[verificar]` **resuelta PARCIALMENTE en T6.6 (2026-07-31)**: las cuentas Business/Professional de Instagram quedan limitadas a la **Meta Sound Collection** (~pistas cleared para uso comercial); la biblioteca de música con licencia "mainstream" es para uso **personal/no-comercial** — mismo patrón que TikTok (Business ⇒ solo catálogo comercial). **Corroborado por ~6 fuentes de terceros independientes**; la página **first-party de Meta existe y es on-topic** (`help.instagram.com/402084904469945`, "Access to the licensed music library on Instagram") pero está **JS-walled** (no alcanzable en texto desde el sandbox), así que NO se confirma con cita first-party. El Advisor ya documenta la restricción como guía in-app. Confirmar el texto first-party de Meta cuando haya una sesión de navegador queda como follow-up.
...
- **Producción (VPS propio, sin GPU)**: `docker-compose.prod.yml` con servicios `web` (Next.js standalone), `worker` (Node + ffmpeg + libass + fuentes + c2patool), `postgres:16` (volumen local, `localhost` only), `caddy` (TLS + reverse proxy + auth básico adicional opcional; `flush_interval -1` en la ruta SSE). Storage en volumen `/data/assets` **montado en `worker` (rw) y en `web` (rw)** — el worker escribe los artefactos que GENERA el pipeline; web escribe los que SUBE el usuario y lee para previews/downloads proxificados. <!-- Reconciliación T0.13 (2026-07-14, regla 6): el borrador decía web `ro` («toda escritura la hace el worker»), pero se escribió antes de que existieran los uploads. Hoy TRES rutas de web escriben en el StorageAdapter: POST /api/assets (T0.5), POST /api/personas/:id/reference-images (T1.5) y storage.delete() al borrar una persona. Son subidas del USUARIO desde el navegador: en una app mono-usuario, encolarlas al worker para respetar la letra del principio sería complejidad sin dueño. Montado `ro`, esas tres rutas mueren con EROFS en cuanto alguien sube un fichero. Se reconcilia el despliegue con lo CONSTRUIDO; el principio útil que sobrevive es el de la ruta de LECTURA: la URL de storage nunca se expone, todo baja proxificado por la app. --> <!-- Reconciliación T0.13 (2026-07-14): el VPS es MULTI-PROYECTO y aporta un Caddy CENTRAL (contenedor edge-caddy en ~/infra/caddy, documentado en ~/AGENTS.md del VPS) que termina TLS para todos los subdominios ⇒ caddy sale del compose del proyecto; web se publica solo en 127.0.0.1:3100 y el flush_interval -1 (SSE) + trust boundary de x-forwarded-for viven en el site file del Caddy central. Cloudflare (proxied) delante del origen. Ajuste de topología de despliegue, no de decisión de producto. -->
...
1. **E2E core**: dada una URL real de una landing propia, obtener variantes aprobadas de 15–30 s con subtítulos karaoke correctos, C2PA firmado y coste total del lote acotado (tier **Premium**) — en < 45 min de reloj con checkpoints atendidos. **La matriz mínima del E2E de fase y su bound de coste están recalibrados a presupuesto real (ver la segunda recalibración abajo).**
   ...
   - **Segunda recalibración — matriz mínima por presupuesto real (2026-07-26): E2E de fase NO VIABLE a $6, pero 1 variante SÍ (smoke-test autorizado)**: el saldo de fal cayó a **~$6** («solo hay $6, no más dinero»). Reproyección sobre la **tabla de pricing REAL del sistema** (`fal-pricing.ts:231` — N7c avatar factura POR SEGUNDO: OmniHuman 16¢/s FIJO en premium `seed-data.ts:637`; N7d b-roll ~20¢/s medido en T4.11). **Clave del coste** (`build-variant-generation-plan.ts:165` + `generate-avatar.ts:2`): el avatar N7c cubre **solo el HOOK** (2–4 s), no la variante entera; el BODY (~12–25 s) va a b-roll N7d. ⇒ coste real por variante ≈ **avatar $0,48 (hook 3 s × 16¢) + b-roll ~$2,4–3,2 (body troceado a buckets [4,6,8] × 20¢) ≈ $2,9–3,7**. **1 variante cabe en $6; las 6 de la cláusula (a) NO** (6 × ~$3,3 ≈ $20, + cláusula b ≈ $23–26 totales). **Correcciones a supuestos previos** (errores del coordinador, cazados por el advisor y confirmados en código): (1) el avatar NO escala con la duración de la variante — cubre el hook; (2) `objective='hook_test'` NO abarata a `hooksPerAngle:1` (`sharedBodyAndCta` comparte por (ángulo,idioma); 1 variante/par → nada que compartir); (3) la cláusula (b) es un lote adicional fuera del bound de (a). **Decisión del usuario 2026-07-26**: gastar los $6 en un **smoke-test de 1 variante real** (idioma es, premium, abort duro si CP3 proyecta >$5) para probar por primera vez que el pipeline compone un máster real + C2PA end-to-end y **medir el coste real por endpoint** post-T5.8c. El smoke-test NO cierra T5.9 (no cubre cláusula a ni c). El E2E de fase completo queda en **parada por prerequisito externo (recarga fal)**; su matriz y bound se fijarán con el coste medido del smoke-test. El bound histórico del lote de 12 variantes sigue siendo **< $40**.
   - **Tercera recalibración — SPLIT del criterio, no rebaja (2026-07-30, saldo real $7,62)**: en vez de dejar 22.1 bloqueado indefinidamente, el usuario decidió re-escalar la vara para que quepa en el techo DURO ($7,62, cero recargas). **22.1 NO se debilita**: se DIVIDE en dos ítems de planning. **T5.9 (re-escalada)** cierra sobre lo verificable en el techo: (a') **1 variante real** compuesta a máster+C2PA vía flujo normal (el máster+C2PA ya se probó el 26/07, `019f9fac-51cc`; la matriz 2áng×3hooks y las captions karaoke son verificables a $0 en CP3/scripted + T5.4); (b) texto libre 0 imágenes (ya era 1 variante); (c') **criterio 22.8 (voces es+en nativas) juzgado sobre el audio de N7b AISLADO** (voiceover `completed`, ~1¢/idioma — la única cláusula nunca satisfecha, ahora al alcance). Proyección ~$3–5, abort duro $5,00 en CP3. **T5.9-full** RETIENE la vara original (≥6 variantes 2áng×3hooks es+en, <$40, matriz completa) como ítem **bloqueado por presupuesto** (~$25–40, fuera del techo) — para que la portada pública NO afirme que el gate de fase pasó a plena fuerza cuando solo pasó la versión re-escalada. La matriz exacta de T5.9-full se fija al autorizar ese gasto.
   - **CIERRE de F5 por decisión del usuario (2026-07-30)**: informado de que solo falta gastar más (~$35–150 incierto, fuera del techo ~$7,43) para ejercer la matriz COMPLETA, el usuario decidió **dar F5 por cerrada** y avanzar de fase. T5.9 queda `[x]` como **cierre por decisión, NO como PASS de 22.1 a plena fuerza**. La matriz ≥6 variantes COMPUESTA end-to-end (el run del 26/07 la ejecutó y FALLÓ 0/6) y la generación real de la variante (b) **NO se han demostrado**; permanecen íntegras en **T5.9-full (⛔)**. El registro de cierre (`docs/verifications/T5.9/report.md`) y los READMEs públicos llevan este matiz — la portada NO afirma que el gate de fase pasó a plena fuerza.
