...
|---|---|---|---|---|
| Login | `/login` | Auth · dos paneles | [`auth.html`](auth.html) | — |
...
| Matriz y confirmación de gasto | CP2 (checkpoint) | Config + rail de coste + matriz | [`batch-matrix.dc.html`](batch-matrix.dc.html) | — |

## Notas de fidelidad

- **`auth.html` (login, T0.4)** — añadido el 2026-07-09 desde el mismo canvas (`Auth.dc.html`, proyecto `1aa625b2-…`). **Desviación acordada con el usuario (2026-07-09):** el mockup dibuja un producto multi-usuario (toggle *Entrar/Crear cuenta*, campo *correo*, *passkey*, *¿Olvidaste?*, *mantener sesión iniciada*, medidor de fuerza de signup). UGC Factory es **mono-usuario self-hosted** (PRD §19.2, skill backend §6): auth = **solo password** (hash sembrado por env en `app_setting`, sesión stateless `exp.hmac`). Al desarrollar `/login` se reproduce **el layout** (dos paneles: formulario a la izquierda + panel técnico decorativo a la derecha con run-log y stats de ejemplo, estética y tokens del DS) pero se **cablea solo**: un campo *contraseña* con toggle ver/ocultar, botón *Entrar*, y zona de error/rate-limit. **Se omiten** signup, correo, passkey, «¿Olvidaste?» y «mantener sesión iniciada» — no crear esa superficie muerta es parte del acuerdo, no un descuido. El reviewer NO debe exigir esas piezas.
...
- **`batch-matrix.dc.html` (CP2, T2.3)** — añadido el 2026-07-14 desde un proyecto propio de Claude Design (`b2efa6c4-…`), generado con el PRD y el planning como fuentes. Es el único mockup que se conserva en formato **`.dc.html` interactivo** (runtime `x-dc` con `<sc-for>`/`<sc-if>` + una clase `DCLogic`): su interactividad ES la spec (el coste recalculándose al cambiar tier, el estado vacío, el confirmado). **Su LAYOUT es vinculante; su `<script>` NO.** Ese script trae un **modelo de coste inventado** —precios de clip hardcodeados, presets de 12/28/48 s, un `compute()` que hace la aritmética en el cliente y un `buildVariants()` que concatena el `filename_code` a mano— y **el backend real dice otra cosa**: los presets son **12/30/45 s** (`packages/core/src/strategy/presets.ts`), el coste sale de `estimateBatchCost` sobre la tabla `recipe` (Apéndice B) y el `filename_code` lo produce `composeMatrix`. En T2.3 **no se portó ni una línea de esa lógica**: el navegador no calcula ni un céntimo (cada cambio de config es un `POST /api/batches/estimate`). **Desviación acordada**: el mockup dibuja un checkbox por HOOK; `ComposeMatrixInput` acepta `angleIndices` + `hooksPerAngle`, así que los hooks se **muestran** en la card y lo que se **selecciona** es el ángulo.
