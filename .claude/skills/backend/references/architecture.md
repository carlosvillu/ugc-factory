...
**Invariante de dinero (no negociable)**: toda llamada de pago va seguida SIEMPRE de su `cost_entry`, en el propio servicio y antes de retornar (*record-first*, T1.4/T1.8) — el registro no puede quedar en manos del llamante, que puede fallar después de haber gastado. **Un executor de pago NUEVO (todo módulo de `packages/services/src` que llame a `recordCost`) DEBE declararse en `packages/services/src/paid-surface-registry.ts`** con su proveedor, nº de llamadas facturadas y unidad de pricing: el test de completitud (`paid-surface-registry.test.ts`, en el gate vía `pnpm test`) falla si cobras sin registrarte. Es el mecanismo que sube este invariante de la prosa al escalón determinista (auditoría 2026-07-23, patrón `gasto-real-sin-ledger`, que reapareció con cada nodo nuevo pese a esta regla). Declarar la entrada es el momento de pensar los invariantes de dinero del executor antes de que fal facture sin `cost_entry`.
