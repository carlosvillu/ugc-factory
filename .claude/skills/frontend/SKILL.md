...
- **Tailwind v4 CSS-first**: no existe `tailwind.config.js`; tokens en `globals.css` (`:root` dark por defecto + `[data-theme="light"]` + `[data-accent=…]` + `@theme inline`), volcados VERBATIM del DS. Solo clases semánticas de token (`bg-surface`, `text-text-2`); colores/espaciados crudos (`text-gray-500`, `#hex`, `bg-white`) prohibidos fuera del fichero de tokens — lo caza el pase `ds-reviewer` (5c) sobre el diff.
