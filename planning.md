...
- **Notas de alcance (T1.10a/T1.10b heredan esto)**:
  ...
  - `ok` se DERIVA de los warnings (`isBlockingWarning`), no es un canal paralelo: para hacer bloqueante un código nuevo basta marcarlo en `BLOCKING_WARNING_CODES`.
  - **Divergencia deliberada con `testing/unit-core.md` §5** (url sin hero: la skill pide "error", aquí es warning tipado bloqueante con `ok=false` — un `ok=false` sin motivo es un fallo silencioso). PRD/planning > skill: **actualizar la skill de forma deliberada** para que no diverja en silencio (regla 6).
...
- ⚠ **DOS BLOQUEOS MEDIDOS EN T1.10a — resolver ANTES de dar por cerrable esta tarea** (evidencia: `docs/verifications/T1.10a/report.md`):
  1. ~~**El pipeline tarda 116,7 s y la Verificación de abajo exige <90 s.**~~ → **RESUELTO 2026-07-12: el bound sube a <180 s** (decisión del usuario, «esperamos sin problemas»; PRD O1 actualizado con su nota). Medido: N1 20,0 s · N2 32,3 s · **N3 64,4 s (55 %)** — el dominante es N3, NO las descargas de N2 como se suponía. Bajar de 90 s exigía **recortar el brief**, degradando justo la pieza de la que depende la calidad de todos los anuncios. Se prefiere un brief bueno en 2 min a uno pobre en 90 s. **NO hay que optimizar nada en T1.10b**; la deuda de `visual-analyze` en serie (~20 s de ganancia) se mantiene anotada pero deja de ser urgente.
