...
**Tokens/utilidades de TD.5 — verificado que NO hay hueco que subir (2026-07-08)**: las foundations que TD.5 creó (`--overlay-strong`, `pulse-ring-static` + vars, `hatch-9x16`/`-wide`, `caption-shadow`) son **mecanismos de nuestro código Tailwind** (`@utility` = constructo de compilación, sin significado en el CSS plano del DS) para reproducir lo que las specs `.jsx` del DS ya expresan **inline** con primitivas existentes (SafeZoneOverlay/VariantCard inline el `repeating-linear-gradient(var(--surface-3) … var(--stripe))`; PipelineNode usa `animation: ugc-pulse-ring`/`ugc-spin`, keyframes ya presentes en `motion.css` remoto). Grep ampliado (var()/keyframes) sobre las specs remotas: ninguna referencia esos tokens; todo lo que referencian ya existe en el DS. No es contenido del DS → no se sube. Espejo local == remoto.
