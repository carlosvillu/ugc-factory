...
**5a · `code-review`** — **usa el subagente `feature-dev:code-reviewer`** (Agent tool) sobre el diff de la tarea, NO el skill `code-review:code-review`: ese skill está cableado a un PR de GitHub (arranca comprobando estado del PR, comenta con `gh`) y este repo **nunca hace push** — es incompatible por construcción. El sustituto revisa el diff local con el mismo filtro de confianza (reporta solo lo alto) y funcionó en T6.2 (0 falsos positivos). Effort proporcional al riesgo: **low** para diffs pequeños/mecánicos (<~200 líneas sin lógica nueva), **medium** por defecto, **high** solo para el orquestador, dinero (spend/fal) y seguridad (auth/webhooks/cifrado). Caza bugs (correctness, robustez, seguridad). Hallazgos de correctness/robustez confirmados → tríalos: **arréglalos si están en alcance** (al implementer vía SendMessage; re-gate tras el fix); **deuda de journal solo si quedan fuera de alcance o son genuinamente diminutos**. `code-review` NO caza bugs por simplicidad — eso es 5b.
