...
- **El heading del control negativo lo parsea un hook — respeta su forma EXACTA o bloquea el cierre pese a tener evidencia válida** (dos incidentes ya: T5.18, T5.21). El hook `guard-planning` busca `## Control negativo` (regex `^#{2,}\s*Control\s+negativo\s*$`) y captura su cuerpo SOLO hasta el próximo heading `#{1,6}`. Dos reglas duras: (1) el heading es LITERAL `## Control negativo`, **sin sufijo** — las aclaraciones van en el cuerpo, no en el heading (T5.18: `## Control negativo (reproducido…)` rompió el match). (2) La prueba de rojo (tokens `FAIL`/`ROJO`/`Expected`/`failed`) debe vivir en el **cuerpo DIRECTO** de `## Control negativo`, ANTES de cualquier subsección `###` — si estructuras la evidencia solo en `### A/### B` debajo, el cuerpo directo queda vacío y el hook lo lee como "sin control negativo" (T5.21). Una línea-resumen con la salida roja arriba, el detalle por subsección debajo.
