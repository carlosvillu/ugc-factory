...
Esta capa cubre los tests de integración de `packages/db` — migraciones Drizzle, constraints, repos tipados, índices — y provee el harness de BD que reutiliza cualquier otro paquete que necesite Postgres de verdad (los tests del orquestador con `SELECT … FOR UPDATE` y `LISTEN/NOTIFY` se montan sobre este mismo harness).

La decisión es vinculante y no se reabre: **nada de mocks de BD ni SQLite**. El producto depende de comportamiento específico de Postgres 16 — `FOR UPDATE`, `NOTIFY`, enums nativos, índices UNIQUE parciales, GIN sobre `text[]`, JSONB, `ON DELETE` — y un doble de test no falla donde Postgres falla. Un test que pasa aquí se comporta igual en el VPS, porque es el mismo motor.
...
5. [Tests de migraciones (T0.3, T1.2)](#migraciones)
6. [Tests de repos tipados](#repos)
7. [Índices GIN con EXPLAIN (T3.1)](#gin)
8. [Pitfalls](#pitfalls)
...
globalSetup (singleton con refcount: 1 contenedor por run)
  └─ postgres:16 (Testcontainer)
       ├─ ugc_template  ← migraciones Drizzle aplicadas; datallowconn=false
       └─ test_a1b2c3, test_d4e5f6, …  ← CREATE DATABASE … TEMPLATE ugc_template
...
export const TEMPLATE_DB = 'ugc_template';
...
export async function startPostgresContainer(): Promise<{
  ...
  // Ruta resuelta respecto al paquete @ugc/db, NUNCA process.cwd(): los
  // scripts por paquete ejecutan vitest desde el directorio del paquete.
  const require = createRequire(import.meta.url);
  await migrate(drizzle(migrator), {
    migrationsFolder: path.join(
      path.dirname(require.resolve('@ugc/db/package.json')),
      'drizzle',
    ),
  });
...
## 5. Tests de migraciones (T0.3, T1.2)
...
```ts
// packages/db/test/integration/brand-kit.constraints.test.ts
import { beforeAll, afterAll, describe, it, expect } from 'vitest';
import { eq } from 'drizzle-orm';
import { createTestDatabase, makeBrandKit, type TestDatabase } from '@ugc/test-utils';
import { brandKit } from '@ugc/db/schema';

let tdb: TestDatabase;
beforeAll(async () => { tdb = await createTestDatabase({ label: 'brand-kit' }); });
...
describe('brand_kit: UNIQUE parcial sobre domain (T1.2)', () => {
  it('admite N filas con domain NULL (modo manual sin dominio)', async () => {
    ...
    await tdb.db.insert(brandKit).values([
  ...
  it('rechaza la segunda fila con el mismo domain', async () => {
    await tdb.db.insert(brandKit).values(makeBrandKit({ domain: 'acme.com' }));
    await expect(
      tdb.db.insert(brandKit).values(makeBrandKit({ domain: 'acme.com' })),
    ).rejects.toThrow(/duplicate key value/); // SQLSTATE 23505
  });

  it('rechaza un valor fuera del enum source', async () => {
    await expect(
      tdb.db.insert(brandKit).values(makeBrandKit({ source: 'scraped' as never })),
    ).rejects.toThrow(/invalid input value for enum/);
  });
});
```

Para las FKs, asserta **la política elegida en la migración** (el PRD exige `ON DELETE` explícito en todas):

```ts
it('borrar url_analysis se comporta como declara la migración', async () => {
  const [ua] = await tdb.db.insert(urlAnalysis).values(makeUrlAnalysis()).returning();
  await tdb.db.insert(productBrief).values(makeBrief({ urlAnalysisId: ua.id }));

  // Si la migración declara CASCADE:
  await tdb.db.delete(urlAnalysis).where(eq(urlAnalysis.id, ua.id));
  expect(await tdb.db.select().from(productBrief)).toHaveLength(0);
...
Los repos concentran el SQL real: `RETURNING`, `ON CONFLICT` (upsert de `brand_kit` por dominio), round-trip de JSONB (`product_brief.data`), defaults y enums. El sistema de tipos no detecta un mapeo camelCase↔snake_case roto ni un default que solo existe en la BD — por eso el roundtrip se hace contra Postgres, no contra un stub.
...
import { createProjectRepo } from '../../src/repos/project';

let tdb: TestDatabase;
beforeAll(async () => { tdb = await createTestDatabase({ label: 'project-repo' }); });
afterAll(async () => { await tdb.close(); });

describe('projectRepo', () => {
  it('create/get hace roundtrip completo (defaults, enums, timestamps)', async () => {
    const repo = createProjectRepo(tdb.db);
    const created = await repo.create(makeProject({ name: 'Demo ES' }));
    const fetched = await repo.getById(created.id);
    // RETURNING y SELECT deben devolver exactamente la misma forma:
    expect(fetched).toEqual(created);
    expect(fetched?.defaultLocale).toBe('es'); // default aplicado por la BD
  });
});
```
...
<a name="gin"></a>
## 7. Índices GIN con EXPLAIN (T3.1)

La verificación de T3.1 pide demostrar que la búsqueda facetada de `prompt_template` usa el GIN. Hay dos estrategias, y conviene entender qué prueba cada una:
...
- **`SET LOCAL enable_seqscan = off`**: fuerza al planner. Solo prueba que el índice *existe y es utilizable* con el operador (`@>` sobre `text[]` casa con la operator class del GIN). Útil como test barato de humo; más débil.

```ts
// packages/db/test/integration/prompt-template.gin.test.ts
import { beforeAll, afterAll, describe, it, expect } from 'vitest';
import { sql } from 'drizzle-orm';
import { createTestDatabase, type TestDatabase } from '@ugc/test-utils';
import { promptTemplate } from '@ugc/db/schema';

const FORMATS = ['grwm', 'unboxing', 'pov', 'demo', 'before-after'];
const ANGLES = ['pain-point', 'question', 'visual-proof', 'social-proof'];

// Determinista: el test puede calcular cuántas filas esperan el filtro.
function seedRow(i: number) {
  return {
    ...
    hookAngles: [ANGLES[i % ANGLES.length]],
    verticals: ['beauty'],
    platforms: ['tiktok'],
  };
}
...
let tdb: TestDatabase;
beforeAll(async () => { tdb = await createTestDatabase({ label: 'gin' }); });
afterAll(async () => { await tdb.close(); });

describe('búsqueda facetada de prompt_template (T3.1)', () => {
  it('usa Bitmap Index Scan sobre el GIN y devuelve exactamente lo esperado', async () => {
    const N = 1500;
    await tdb.db.insert(promptTemplate).values(
      Array.from({ length: N }, (_, i) => seedRow(i)),
    );
    ...
    const plan = await tdb.db.execute(sql`
      ...
      WHERE formats @> ARRAY['grwm'] AND hook_angles @> ARRAY['pain-point']
    `);
    ...
    const expected = Array.from({ length: N }, (_, i) => seedRow(i))
      .filter((r) => r.formats.includes('grwm') && r.hookAngles.includes('pain-point'))
      .length;
  ...
  it('variante barata: el índice es utilizable (enable_seqscan=off)', async () => {
    ...
    await tdb.db.transaction(async (tx) => {
      ...
      const plan = await tx.execute(sql`
        EXPLAIN (FORMAT JSON)
        SELECT id FROM prompt_template WHERE formats @> ARRAY['grwm']
      `);
      expect(JSON.stringify(plan.rows)).toMatch(/Bitmap Index Scan/);
    });
  });
...
- [ ] ¿Nueva tabla o migración? → test de constraints en `packages/db/test/integration/` que fije UNIQUE (incl. parciales), enums y `ON DELETE` observables (patrón T1.2).
- [ ] ¿Nuevo repo? → roundtrip real create/get/update + casos de conflicto (`ON CONFLICT`, violaciones UNIQUE) contra el clon.
- [ ] ¿Nuevo índice para una query caliente? → test EXPLAIN con volumen sintético + assert de corrección del resultado (patrón T3.1).
...
- [ ] Datos de prueba vía factories (`makeProject()`, `makeBrief()`, `makeVariant()`, …) y `seedFixtures(db)` de `@ugc/test-utils`, nunca literales repetidos: cuando el schema evolucione, se arregla la factory, no cincuenta tests.
