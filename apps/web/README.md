...
| `/gallery`       | La galería facetada de templates de prompt: ficha con slots, guards y versiones; editor con validación en vivo y diff |
...
| `/api/templates` · `/api/templates/[id]`           | `GET`, `POST` · `GET`, `PATCH`                 |
| `/api/templates/[id]/status`                       | `PATCH` — transición draft/review/published    |
...
- `src/components/` — `ui/` (design system), `run-canvas/`, `checkpoints/`, `intake/`, `personas/`, `gallery/`, `runs/`, `settings/`, `spend/`
