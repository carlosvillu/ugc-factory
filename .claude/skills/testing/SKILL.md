...
9. **EL ARNÉS NUNCA PUEDE SER MÁS CÓMODO QUE LA REALIDAD.** Es el anti-patrón que más caro nos ha salido: **≥10 incidentes documentados** (la auditoría del journal de 2026-07-23 los contó; siguió apareciendo en F4 y F5 tras escribirse esta regla), en capas distintas, y en todos la suite estaba **verde mientras la funcionalidad estaba rota**. Por eso el control negativo pasó a ser un gate mecánico (ver el cierre de este principio). Las seis formas que adopta:
   ...
   **AHORA ES OBLIGATORIO Y MECÁNICO (auditoría 2026-07-23):** este anti-patrón reapareció ≥10 veces PESE a vivir en esta prosa, así que el control negativo dejó de depender del juicio. El hook `guard-planning.sh` **bloquea el cierre** (`[x]` en `planning.md`) si el `report.md` de la tarea no trae una sección `## Control negativo` con evidencia de un test puesto en ROJO al reintroducir el bug — o un `N/A — <motivo>` auditable para tareas sin superficie de test. El report DEBE incluir esa sección; el verifier la produce al ejecutar la Verificación. La prosa de arriba explica CÓMO hacer un buen control negativo; el hook garantiza que exista.
