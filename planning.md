...
#### T1.14 · El filtro de imágenes deja fuera AVIF y las URLs sin extensión
- **Depende de**: —
...
#### T1.15 · Perfil `url` sin hero image: decisión de CP1, no fallo del run
- **Depende de**: T1.11
...
#### T1.16 · Nodos con título humano + visor modal del output JSON
- **Depende de**: —
...
- **Playwright permanente**: `runs-canvas.spec.ts` extiende: el nodo N2 muestra su título humano; click en la caja de output → modal con el JSON completo (un campo que el excerpt trunca es visible en la modal) → copiar y cerrar.
