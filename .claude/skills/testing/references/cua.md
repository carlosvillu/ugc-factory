...
**Regla del proyecto**: ninguna tarea del `planning.md` se marca `[x]` sin haber ejecutado su campo **"Verificación"** contra el sistema real levantado, y sin haber persistido la evidencia en `docs/verifications/<TASK-ID>/`. Esto es un *gate*, no una sugerencia: es la materialización de la filosofía baby-steps del planning ("no 'el código compila', sino 'hago X y veo Y'") y de la regla de trabajo 2 ("verificación ejecutada y anotada").

CUA = *Computer-Using Agent*: un agente (tú) que opera la aplicación **como lo haría un humano**, con el CLI `agent-browser` (Vercel Labs), reproduciendo el flujo descrito en la Verificación de la tarea y comprobando lo **observable** — el nodo del canvas cambia de color, el coste aparece en `/spend`, el download llega con el checksum correcto.

## Por qué existe este gate

- Los tests (unit, integración, E2E) demuestran que el código hace lo que sus tests dicen. El gate CUA demuestra que **el sistema entero** (docker compose + web + worker + Postgres + SSE atravesando el stack real) hace lo que la tarea prometía. Son fallos de categorías distintas: un `transition()` perfecto en Vitest puede convivir con un canvas que nunca se actualiza porque Caddy bufferiza el SSE.
...
- El lector futuro (humano o agente) de `docs/verifications/` puede reconstruir qué funcionaba, cuándo y a qué coste — imprescindible en un proyecto donde las verificaciones de F1+ gastan dinero real (regla de trabajo 5: coste real anotado; si difiere >25 % del estimado, se recalibra en la misma tarea).
...
Regla práctica: cuando un flujo se verifica por CUA y va a repetirse (los E2E de fase T1.10b, T2.6, T4.11, T5.9 son candidatos obvios), destila después un spec de Playwright que congele lo esencial. El CUA descubre; el E2E vigila. Ver `e2e.md`.

## El flujo completo, paso a paso

### Paso 0 — Decide la superficie: ¿UI, solo backend o cliente MCP?

Lee la Verificación de la tarea en `planning.md`:

- **Tiene superficie UI** (menciona "en el navegador", "el canvas", "el panel", "click", "ver en vivo"...): gate CUA con agent-browser. Ejemplos: T0.11, T1.10b, T2.3, T5.6.
- **Solo backend** (menciona `curl`, `psql`, scripts, logs): no fuerces un navegador donde no aporta. Ejecuta el script/curl **contra el sistema levantado** (nunca contra mocks) y captura la salida como evidencia. Ejemplos: T0.7a (script de transiciones + `psql LISTEN pipeline_events`), T0.10 (`curl -N` del SSE), T1.9 ("a nivel de datos, sin UI"). **El report en `docs/verifications/` se escribe igual** — el gate es la evidencia, no el navegador.
- **Tool MCP (T8.5)**: la verificación se hace desde un **cliente MCP real** — la tool se invoca desde Claude Code de verdad (no desde un harness que simule el protocolo) y el efecto se observa en el canvas con agent-browser. La evidencia se captura igual: output de la invocación + screenshots del canvas en `docs/verifications/T8.5/`.

Muchas tareas mezclan superficies (T0.13: `https://<dominio>` en navegador + `pg_restore --list` en terminal, y su objetivo es el VPS — ver la subsección del Paso 1): cubre cada punto con la herramienta que la Verificación describe.
...
pnpm db:migrate
pnpm seed                                        # seeds que la tarea necesite (personas, galería...)
pnpm dev                                         # web + worker, en otra terminal o en background
curl -s localhost:3000/api/health                # gate de arranque: {ok:true, db:true}
```

Por qué: un CUA contra un sistema a medias produce falsos FAIL (worker caído = steps eternamente `queued`) o, peor, falsos PASS (verificar contra un `pnpm dev` de hace 3 días que no incluye tu cambio). Antes de empezar, confirma que el código que corre es el que verificas (`git status` limpio o anota el diff en el report).

**Preparar datos por API/seed está permitido y es recomendable** — solo el flujo bajo verificación debe ser humano (ver Reglas de oro). Ej.: para verificar CP3 (T2.6) puedes crear el run y aprobarle CP1/CP2 por API si esos checkpoints ya fueron verificados en sus tareas; lo que no puedes es aprobar CP3 por API.

#### Verificaciones cuyo objetivo es el VPS (T0.13, T4.2, T8.1)
...
- **Curl/navegador desde fuera del VPS**: T0.13 pide comprobar `https://<dominio>` (certificado incluido) y que el SSE atraviesa Caddy — eso se verifica desde tu máquina u otra red, no desde localhost del VPS.
- **Restore drill (T8.1)**: restaurar el backup en un **contenedor limpio** (no sobre la BD viva) y arrancar la app desde él; la evidencia es el output del restore + la app funcionando contra la BD restaurada.
...
Ejemplo real — T0.11 (canvas React Flow v1); la Verificación pide *ver los nodos cambiar de color en vivo*, aprobar un checkpoint, provocar un fallo, retry y cancelar:
...
npx -y agent-browser fill @e3 "$UGC_DEV_PASSWORD"
npx -y agent-browser click @e5
npx -y agent-browser wait --url "**/"     # sesión iniciada

# Lanza el run de demo DESDE LA UI y captura la URL resultante:
npx -y agent-browser snapshot -i                    # refs frescos del dashboard
npx -y agent-browser click @e8                      # "Nuevo run demo"
npx -y agent-browser wait --url "**/runs/*"
npx -y agent-browser get url                        # anota el /runs/<run-id> creado (va al report)
npx -y agent-browser screenshot "$EV/01-run-inicial.png"
npx -y agent-browser wait --text "running"          # SIN reload: el punto es el SSE
npx -y agent-browser screenshot "$EV/02-nodo-running.png"
npx -y agent-browser wait --text "waiting_approval"
npx -y agent-browser snapshot -i                    # refs frescos del panel del checkpoint
npx -y agent-browser click @e12                     # "Aprobar y continuar"
npx -y agent-browser wait --text "succeeded"
npx -y agent-browser screenshot "$EV/03-aprobado-y-completado.png"
...
- **Verifica lo observable, literalmente.** Si la Verificación dice "ver los nodos cambiar de color en vivo", **no hagas `reload`** entre estados: un reload enmascararía un SSE roto con el fallback de revalidación. Si dice "el coste aparece en `/spend`", navega a `/spend` y lee la cifra (`get text`), no consultes la tabla `cost_entry` por psql — eso verifica la BD, no la feature.
- **Waits correctos, no `wait 2000`.** Usa `wait --text`, `wait --url` o `wait <selector>` según el caso (la skill del CLI lo detalla). **Nunca `wait --load networkidle` en páginas con SSE vivo**: el canvas de `/runs/*` mantiene la conexión abierta (heartbeat incluido), networkidle no se alcanza jamás y el comando cuelga hasta el timeout. Un CUA flaky por waits malos genera FAILs falsos que erosionan la confianza en el gate.
...
- **Contraste de texto (aserción obligatoria en cualquier verificación de UI, clarificación 2026-07-08)**: no basta con "se ve bien" ni con medir el color de fondo — **mide el contraste real texto/fondo** de los elementos con color de acento/semántico (botones, badges, chips, alerts), en dark Y light Y en cada acento que la tarea toque, con `getComputedStyle` (color + background) y el ratio WCAG. Umbral: **4.5:1** texto normal, **3:1** texto grande/negrita. Por debajo = FAIL (o hallazgo a rutear si el color viene del DS: el defecto está en los valores del DS, decisión del usuario, pero se REPORTA con la tabla de ratios, no se ignora). Origen: TD.7 — el botón `primary` renderizaba texto casi-negro sobre indigo en light (bug de `tailwind-merge` que borraba `text-text-on-accent`) y, ya corregido, el blanco sobre emerald/amber/cyan seguía fallando AA (2.1–2.5:1). NINGÚN verifier de TD.1–TD.7 lo cazó porque medían fondos y tokens, nunca el contraste del texto sobre el acento. Es un agujero del protocolo que F0 no debe repetir.
...
- **Outputs de terminal** para la parte backend: `curl ... | tee "$EV/sse-stream.txt"`, salida del script de smoke, `psql` de la query de comprobación.
...
Estos artefactos se commitean: son la memoria del proyecto (y la fuente para la conciliación de costes de T7.6).
...
```markdown
#### T0.11 · Canvas React Flow v1 [x] 2026-08-14 — PASS, ver docs/verifications/T0.11/
```
...
1. **El CUA usa la app como un humano.** Para el paso que se está verificando: nada de atajos por API, ni `eval` que simule el click, ni tocar la BD para "avanzar" el run. Si el botón no es clicable por el agente, probablemente tampoco por el usuario — eso ES un hallazgo. Los atajos sí valen para **preparar** el escenario (seeds, crear el run por API, login vía `state save` de una sesión previa).
...
4. **No rebajes la Verificación.** Si el texto del planning pide "20 runs concurrentes", son 20, no 3. Los E2E de fase son sagrados (regla de trabajo 4): no se marcan por aproximación. Si la Verificación resulta imposible tal cual está escrita, eso es un cambio de alcance: se edita planning/PRD en la misma sesión (regla 6), no se finge.
5. **Presupuesto consciente.** Verificaciones de F1+ llaman a Anthropic/fal/Firecrawl de verdad: usa el tier barato que la tarea permita, anota cada dólar y contrasta con `/spend` — la propia página de gasto es parte de lo observable en muchas tareas. (La suite de tests, en cambio, jamás gasta: los mocks viven en `packages/test-utils`, ver `external-apis.md`.)
6. **El gate no sustituye a los tests.** Llegar al CUA con la suite en rojo es hacer trampa al orden: `pnpm test` verde primero (ver `db-integration.md` para la capa de integración), CUA después. El CUA valida el sistema; los tests validan que puedas tocarlo mañana.
