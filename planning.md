...
#### T1.18 · Una candidata a hero que no se puede descargar no debe ofrecerse
- **Depende de**: T1.15
...
- **Entrega**: que CP1 no ofrezca como promovible una imagen que el sistema no puede usar. Dos caminos (decide el implementer, argumentando): (a) **proxy de miniaturas** — servir las candidatas por el proxy de assets propio (T0.5 ya proxifica descargas), que además arregla la miniatura rota; o (b) **pre-check** al construir el brief (N2 ya descarga cada imagen: puede marcar en el contrato cuáles bajó de verdad, y CP1 ofrece solo esas). (b) es más barato y reusa trabajo YA hecho; (a) arregla además lo visual. **No inventar un tercer canal**: la clasificación de N2 ya viaja con cada imagen.
